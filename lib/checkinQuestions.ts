...
export function generateLifestyleMessage(answers: CheckinAnswers): string {
    ...
    if (score >= 80) {
        return `🌟 Excellent! Your lifestyle score is ${score}/100. You're making great health choices today!`;
    }

    if (score >= 60) {
        return `✅ Good! Your lifestyle score is ${score}/100. Keep building these healthy habits.\n\n${insights.slice(0, 1).join('\n')}`;
    }

    if (score >= 40) {
        return `📊 Your lifestyle score is ${score}/100. Small improvements add up:\n\n${insights.slice(0, 2).join('\n')}`;
    }

    return `💙 Your lifestyle score is ${score}/100. Let's work on building healthier habits:\n\n${insights.join('\n')}`;
}
